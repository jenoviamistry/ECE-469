...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &op)) < 0) return r;
	r = file_read(op->o_file, ret->ret_buf, req->req_n, op->o_fd->fd_offset); // read file
	if (r<0) return r;
	op->o_fd->fd_offset = op->o_fd->fd_offset + r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// is the file open
	if ((ret = openfile_lookup(envid, req->req_fileid, &op)) < 0) return ret;
	ret = file_write(op->o_file, req->req_buf, req->req_n, op->o_fd->fd_offset); // write to file
	if (ret < 0) return ret;
	op->o_fd->fd_offset = op->o_fd->fd_offset + ret;
