...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
		...
		uint32_t *indirectPt = (uint32_t*)diskaddr(f->f_indirect);
		*ppdiskbno = &indirectPt[filebno - NDIRECT];
...
{
    	// LAB 5: Your code here.
		uint32_t *diskbno; // disk block no 
		int r;

		if ((r = file_block_walk(f, filebno, &diskbno, true)) < 0) return r;
		if (*diskbno == 0)
		{
